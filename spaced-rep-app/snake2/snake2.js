/**
 * "Snake 2" — the same game as snake/, drawn in SVG instead of on a canvas.
 *
 * It exists to be measured against the original: same rules, same state, same
 * pixels as far as they can be made to match, so the only thing the numbers can
 * differ by is the renderer. Everything outside view.draw is snake's, generated
 * from it rather than copied by hand.
 *
 * The scene is rebuilt from scratch on every frame, exactly as the canvas
 * clears and repaints on every frame. Keeping nodes and touching only what
 * moved would be the faster way to drive SVG, and a different experiment: this
 * one asks what the same work costs in the two technologies.
 *
 */
/**
...
function snake2(container) {
     ...
     */
    // No equivalent here on purpose: an SVG has no raster surface of its own
    // to make finer. The browser rasterises it at whatever size it is shown,
    ...
    function view(host) {
        ...
        view.mount = (opts) => {
            ...
            svg = $(host, `<svg class="snake-svg" id="snake-svg" viewBox="0 0 ${side} ${side}" style="background: ${palette.canvasBg}; border: 1px solid ${palette.canvasBorder}; border-radius: 14px; box-shadow: 0 4px 12px rgba(0,0,0,${palette.canvasShadow}); display: block; touch-action: none; width: 100%; height: auto; aspect-ratio: 1; cursor: pointer;">
            ...
            svg.addEventListener('click', () => cb.onCanvasClick());
        ...
        // rather than its em box — the same correction the canvas version makes,
        // for the same reason: the ink of a comma sits at the bottom of the box.
    ...
    snake2.render = render;

    snake2.setTheme = (isDark) => {
    ...
    snake2.start = (setId, allowEarly, previous) => {
